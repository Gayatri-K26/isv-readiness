...
6. Order gap rows from the normalized provider execution plan: declared phase
   order first, then provider step order, with unwired suite steps kept in their
...
10. Ignore output references made only by skipped command steps. Since those
    consumers never execute, their arguments cannot impose runtime output
    requirements on earlier producer scripts.
...
- `uv run python -m unittest discover -s tests`: 163 tests passed.
