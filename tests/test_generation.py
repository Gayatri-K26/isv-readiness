...
class GeneratorAdapterTests(unittest.TestCase):
    ...
    def test_generator_receives_contract_and_returns_hash_bound_change_set(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertIn("use exactly one provider-shared client", skill)
            self.assertIn("documented already-absent result", skill)
            self.assertIn("cleanup_errors", skill)
            ...
            self.assertIn("additional provider-owned lifecycle scripts", skill)
            self.assertNotIn("Edit only the selected configuration step", skill)
